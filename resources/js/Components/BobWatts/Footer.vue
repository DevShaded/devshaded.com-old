...
const navigation = {
    ...
    social: [
        ...
        {
            name: 'GitHub',
            href: 'https://github.com/DevShaded/Bob-Watts',
            icon: defineComponent({
                render: () =>
                            h('svg', { fill: 'currentColor', viewBox: '0 0 24 24' }, [
                                h('path', {
        ...
        <div class="max-w-7xl mx-auto py-12 px-4 overflow-hidden sm:px-6 lg:px-8">
            <nav class="-mx-5 -my-2 flex flex-wrap justify-center" aria-label="Footer">
                <div v-for="item in navigation.main" :key="item.name" class="px-5 py-2">
                    <Link :href="item.href" class="text-base text-gray-400 font-medium hover:text-gray-300">
                        {{ item.name }}
                    </Link>
                </div>
            </nav>
            ...
            <p class="mt-8 text-center text-base text-gray-400 font-medium">&copy; {{ new Date().getFullYear() }}
                Bob Watts. All rights reserved.</p>
        </div>
    </footer>
</template>
